...
class Decision(Page):
    ...
    def before_next_page(self):
        ...
        for n, pref in zip(indices, form_fields):
            ...
            self.participant.vars['player_prefs'][current_round][n - 1] = [int(choice_i)]
...
class Results(Page):
    ...
    def vars_for_template(self):
        current_round = self.subsession.round_number
        player_prefs = [i[0] for i in self.participant.vars['player_prefs'][current_round]]
        successful = [i for i in self.participant.vars['successful'][current_round]]

        return {
                'player_prefs': player_prefs[current_round],
                'successful': successful[current_round],
                'indices': [j for j in range(1, Constants.nr_courses + 1)],
                'valuations': self.participant.vars['valuations'][current_round]
                }
